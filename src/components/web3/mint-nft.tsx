...
import { CopyIcon, ExclamationTriangleIcon, ReloadIcon, ChevronDownIcon, ChevronUpIcon } from "@radix-ui/react-icons"

type SalePhase = "not-started" | "allowlist" | "public" | "paused" | "sold-out" | "ended"
type MintStatus = "idle" | "approving" | "minting" | "confirming" | "success" | "error"
...
export function MintNFT({
  ...
  const [walletAddress] = useState("0x742d35Cc6634C0532925a3b844Bc9e7595f4e89")
  const [balance] = useState(2.5) // ETH balance
  ...
  const getSaleStatusBadge = () => {
    const variants: Record<SalePhase, { label: string; variant: "default" | "secondary" | "destructive" | "outline" }> =
      {
  ...
  const canMint = () => {
    ...
    if (wrongNetwork) return { can: false, reason: "Switch to Ethereum Mainnet" }
    if (salePhase === "not-started") return { can: false, reason: "Sale hasn't started yet" }
    if (salePhase === "paused") return { can: false, reason: "Minting is paused" }
    if (salePhase === "sold-out") return { can: false, reason: "Collection sold out" }
    if (salePhase === "ended") return { can: false, reason: "Sale has ended" }
    if (!canAfford) return { can: false, reason: "Insufficient funds" }
    if (!withinWalletLimit) return { can: false, reason: `Wallet limit reached (${maxPerWallet} max)` }
    if (!withinTxLimit) return { can: false, reason: `Transaction limit exceeded (${maxPerTx} max)` }
    if (needsApproval && !isApproved) return { can: false, reason: "Approve tokens first" }
          ...
          <Card>
            <CardContent className="p-4 md:p-6">
              ...
              <div className="relative aspect-square overflow-hidden rounded-lg bg-muted">
                <img src="/cool-cat-nft.jpg" alt={collectionName} className="h-full w-full object-cover" />
              ...
              <div className="mt-4 space-y-3">
                <div>
                  <h2 className="text-xl font-bold md:text-2xl">{collectionName}</h2>
                  <p className="text-sm text-muted-foreground">{collectionSymbol}</p>
                </div>
                <p className="text-sm text-muted-foreground">{description}</p>

                {/* Creator Info */}
                <div className="flex items-center gap-3 rounded-lg border border-border p-3">
                  <img src={creatorAvatar || "/placeholder.svg"} alt={creatorName} className="h-10 w-10 rounded-full" />
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <Button
                      ...
                      size="icon"
                      className="h-8 w-8"
                      onClick={() => window.open("https://twitter.com/coolcatsnft", "_blank")}
                    >
                      <svg
                        className="h-4 w-4"
                        viewBox="0 0 24 24"
                    ...
                    <Button
                      ...
                      size="icon"
                      className="h-8 w-8"
                      onClick={() => window.open("https://coolcatsnft.com", "_blank")}
                    >
                ...
                <div className="flex items-center gap-2 rounded-lg border border-border p-3">
                  ...
                  <code className="flex-1 truncate text-xs">{contractAddress}</code>
                  <Button
                    variant="ghost"
                    size="icon"
        ...
        <div className="space-y-4 md:space-y-6">
              ...
              <CardContent className="p-4 md:p-6">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground">Please switch to Ethereum Mainnet</p>
                  </div>
              ...
              {salePhase === "not-started" && (
                <div className="flex items-center gap-2 rounded-lg bg-muted p-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-lg font-bold text-primary">{formatTime(countdown)}</p>
                  </div>
                </div>
              )}
          ...
          <Card>
            <CardHeader className="p-4 md:p-6">
              ...
              <CardDescription>Select quantity and mint your NFTs</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 p-4 md:space-y-6 md:p-6 md:py-0">
              ...
              <div className="rounded-lg border border-border p-4">
                <div className="flex items-baseline justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm text-muted-foreground">≈ ${(price * 3500).toFixed(2)} USD</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground md:text-sm">Max {maxPerTx} per transaction</span>
                </div>
                <Input
                  type="number"
                  min={1}
                  max={maxPerTx}
                  value={quantity}
                  onChange={(e) => setQuantity(Math.min(maxPerTx, Math.max(1, Number.parseInt(e.target.value) || 1)))}
                  className="text-center text-lg font-semibold"
                />
                <BalanceQuickSelect
                  balance={String(maxPerTx)}
                  onAmountSelect={(amount) => setQuantity(Math.floor(Number(amount)))}
                />
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Subtotal ({quantity} NFTs)</span>
                  <span className="font-medium">{totalCost.toFixed(4)} ETH</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Estimated gas</span>
                  <span className="font-medium">{estimatedGas.toFixed(4)} ETH</span>
                </div>
                <Separator />
                <div className="flex items-center justify-between">
                  <span className="font-medium">Total</span>
                  <div className="text-right">
                    <p className="text-lg font-bold md:text-xl">{totalWithGas.toFixed(4)} ETH</p>
                    <p className="text-sm text-muted-foreground">≈ ${(totalWithGas * 3500).toFixed(2)} USD</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between rounded-lg bg-muted p-3">
                ...
                <span className={cn("text-sm font-medium", !canAfford && "text-destructive")}>
                ...
                <CollapsibleContent className="space-y-4 pt-4">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm text-muted-foreground">{["Eco", "Standard", "Fast"][gasPriority]}</span>
                    </div>
                    <Slider
                      value={[gasPriority]}
                ...
                <div className="space-y-3 rounded-lg border border-border p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium">
                        ...
                        {mintStatus === "confirming" && "Confirming transaction..."}
                      </p>
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3, 4].map((step) => (
                      <div
                        key={step}
                        className={cn("h-1 flex-1 rounded-full", step <= getStatusStep() ? "bg-primary" : "bg-muted")}
                      />
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-2">
                {needsApproval && !isApproved && (
                  <Button className="w-full" size="lg" onClick={handleApprove} disabled={mintStatus === "approving"}>
                ...
                >
                  {mintStatus === "minting" || mintStatus === "confirming" ? (
                    ...
                    <>
                      ...
                      {mintCheck.can ? `Mint ${quantity} NFT${quantity > 1 ? "s" : ""}` : mintCheck.reason}
                    </>
                  )}
                </Button>
              </div>
              ...
              {!mintCheck.can && (
                <div className="flex items-start gap-2 rounded-lg bg-muted p-3">
                  ...
                  <p className="text-sm text-muted-foreground">{mintCheck.reason}</p>
                </div>
              )}
            </CardContent>
          </Card>
      ...
      <Dialog open={showSuccess} onOpenChange={setShowSuccess}>
        <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-md">
          <DialogHeader>
            ...
            <DialogTitle className="text-center text-xl md:text-2xl">Mint Successful!</DialogTitle>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {mintedTokenIds.map((tokenId) => (
                <div key={tokenId} className="aspect-square overflow-hidden rounded-lg border border-border">
                  <img
            ...
            <div className="rounded-lg bg-muted p-3">
              ...
              <p className="text-sm text-muted-foreground">{mintedTokenIds.join(", ")}</p>
            </div>
